...
//BARRIERS
    ...
    this.add.line(0, 0, 300, 100, 600, 100, '0xff0000').setOrigin(0,0);
    ...
    gameState.barriers.create(550, 100, 'wideBarrierNew');
  ...
  update() {
    ...
    //barrier movement logic if level is still going
      ...
      gameState.barriers.getChildren().forEach(function(item, index) {
        ...
        else if (item.state === 2 && (item.x <= 300 && item.x >= 270)) {
          item.state = 1;
        }

        //states 11-20 reserved for barrier2
        if (item.state === 11 && (item.y <= 630 && item.y >= 600)) {
          item.state = 12;
        }
        else if (item.state === 12 && (item.y <= 130 && item.y >= 100)) {
          item.state = 11;
        }
      });
      ...
      if (gameState.bulletsRemaining <= 0 && gameState.bullet1inScreen == false && gameState.bullet2inScreen == false && gameState.activatedBarriers != gameState.numOfBarriers) {
        this.scene.pause();
